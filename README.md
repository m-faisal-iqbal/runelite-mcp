...
4. Use `agent_start_goal`, `agent_status`, `agent_stop`, `agent_pause`, `agent_resume`, and `agent_history` to manage an in-memory agent session. Use `agent_run_goal` for the Phase 2 bounded observe-plan-execute-verify loop; real autonomy requires `executionMode: "execute"` plus `confirmExecution: "RUN_AUTONOMY"`.

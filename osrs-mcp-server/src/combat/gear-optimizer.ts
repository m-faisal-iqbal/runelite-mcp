...
export type OwnedItem = { name: string; id?: number; quantity: number };
export type GearSlot = "weapon" | "shield" | "head" | "body" | "legs" | "hands" | "feet" | "neck" | "ring" | "ammo" | "cape";
...
type TierItem = { name: string; tier: number; req: number };

function bestOwned(ownedNames: Set<string>, tierList: TierItem[], levelReq: number): TierItem | undefined {
  return tierList
...
export function recommendGear(
  ...
  const add = (slot: GearSlot, item: TierItem | undefined, reason: string) => {
    if (item) recs.push({ slot, item: item.name, reason, tier: item.tier });
  };

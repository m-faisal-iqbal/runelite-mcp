...
export type OfferStatus = "EMPTY" | "PENDING" | "PARTIAL" | "COMPLETE" | "CANCELLED";

export type GeOffer = {
  slot: number;         // GE slot 0-7 (8 slots for members, 3 for F2P)
  type: OfferType;
  itemName: string;
  itemId: number;
  quantity: number;
  price: number;        // per-item price set
...
export type PlaceOfferArgs = {
  type: OfferType;
  itemName: string;
  itemId: number;
  quantity: number;
  price: number;        // per-item price to offer/ask
  slot?: number;        // auto-select if omitted
};
...
export function openGeSlot(slot: number): OfferAction {
...
export function clickOfferType(type: OfferType): OfferAction {
  return {
...
export function setQuantity(quantity: number): OfferAction {
  return {
    action: "set_quantity",
...
export function setPrice(price: number): OfferAction {
  return {
    action: "set_price",
    tool: "handle_dialogue",
...
export function confirmOffer(): OfferAction {
  return {
...
export function buildPlaceOfferActions(args: PlaceOfferArgs): OfferAction[] {
...
export function buildCollectActions(slot: number): OfferAction[] {
  return [
    openGeSlot(slot),
...
export function buildCancelActions(slot: number): OfferAction[] {
  return [
    openGeSlot(slot),
...
export function parseOffersFromSnapshot(snapshot: {
  ...
  if (!geGroup || !Array.isArray(geGroup)) return null;

  const offers: GeOffer[] = [];
  for (let slot = 0; slot < 8; slot++) {
    const childBase = GE_SLOT_CHILD_OFFSET + slot * 6;
    const child = (geGroup as unknown[])[childBase];
    ...
    offers.push({
      slot,
      type:           String(c.type ?? "BUY").toUpperCase() as OfferType,
...
function parseOfferStatus(raw: string): OfferStatus {
...
export function findEmptySlot(offers: GeOffer[]): number | null {

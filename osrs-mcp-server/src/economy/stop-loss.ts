...
export type BudgetConfig = {
  sessionBudget:    number;   // max GP to spend this session
  stopLossGp:       number;   // halt if net loss exceeds this
  stopLossPercent:  number;   // halt if loss > X% of starting GP
  profitTarget:     number;   // optional: stop when profit reaches this (0 = disabled)
  maxFlipsPerHour:  number;   // rate limit on offer placements
  maxActiveOffers:  number;   // max simultaneous GE offers
};

export type BudgetState = {
  sessionStartGp:   number;
  currentGp:        number;
  gpSpent:          number;
  gpEarned:         number;
  netPnl:           number;
  flipsThisHour:    number;
  activeOffers:     number;
...
export function createBudgetState(startGp: number): BudgetState {
  return {
...
export function recordSpend(state: BudgetState, amount: number): BudgetState {
  return {
    ...state,
...
export function recordEarning(state: BudgetState, amount: number): BudgetState {
  return {
    ...state,
    currentGp:   state.currentGp + amount,
...
export function resetHourlyFlips(state: BudgetState): BudgetState {
...
export function checkBudget(
  state:  BudgetState,
  config: BudgetConfig,
  cost:   number        // GP cost of the proposed offer
): BudgetCheckResult {
...
export function formatSessionSummary(state: BudgetState): string {

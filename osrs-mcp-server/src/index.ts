...
function actionStep(tool: string, reason: string, args: Record<string, any>, extra: Record<string, any> = {}) {
  return {
    tool,
    reason,
    arguments: args,
...
function buildNextActionPlan(context: any, snapshot: RuneLiteSnapshot, objective?: string) {
  const state = snapshot.state ?? {};
  const dialogueType = snapshot.interfaceSummary?.dialogueType ?? snapshot.dialogue?.type ?? "NONE";
  const hpPercent = healthPercent(snapshot);

...
import { getQuestDefinition, QUEST_REGISTRY, type QuestDefinition, type QuestStage } from "./quest-registry.js";

export type QuestStatus = "NOT_STARTED" | "IN_PROGRESS" | "COMPLETED" | "UNKNOWN";

export type QuestStateResult = {
  questId: string;
  questName: string;
  status: QuestStatus;
  currentVarbitValue: number | null;
...
function readVarbit(snapshot: RuneLiteSnapshot, varbitId: number): number | null {
...
function resolveStage(varbit: QuestStage, currentValue: number) {
...
export function getQuestState(questName: string, snapshot: RuneLiteSnapshot): QuestStateResult {
...
export function isQuestComplete(questName: string, snapshot: RuneLiteSnapshot): boolean {
  return getQuestState(questName, snapshot).status === "COMPLETED";
}

/** Get next action for any quest in progress */
export function getNextQuestAction(questName: string, snapshot: RuneLiteSnapshot): {
  action: string;
  location?: string;
  items?: string[];
  npcs?: string[];
...
export function countQuestPoints(snapshot: RuneLiteSnapshot): number {
  let total = 0;

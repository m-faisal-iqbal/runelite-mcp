...
// ─── Inventory helpers ────────────────────────────────────────────────────────
function getInventoryNames(snapshot) {
...
function getSkillLevel(snapshot, skill) {
    const skills = snapshot?.skills;
    if (!skills)
        return 1;
    const val = skills[skill.toLowerCase()];
    if (!val)
        return 1;
    if (typeof val === "number")
        return val;
    if (typeof val === "object")
        return Number(val.level ?? 1);
    return 1;
}
function hasRunesFor(inventory, runeNames) {
    return runeNames.every((rune) => inventory.some((item) => item.includes(rune.toLowerCase().replace(" rune", ""))));
}
function findTeleportItem(inventory) {
...
// ─── Build escape plan ────────────────────────────────────────────────────────
export function buildEscapePlan(snapshot, wildernessLevel) {
 ...
 */
export function getBestEscapeAction(snapshot, wildernessLevel) {
 ...
 */
export function checkAndEscape(snapshot) {
    const wildy = getWildernessLevelFromSnapshot(snapshot);
    if (!wildy.inWilderness)
        return null;
...
export function formatEscapePlan(plan) {
    const lines = [

...
export function getQuestDefinition(name) {
    const norm = name.toLowerCase().replace(/[^a-z0-9]/g, "_").replace(/_+/g, "_");
    return (QUEST_REGISTRY[norm] ??
        Object.values(QUEST_REGISTRY).find((q) => q.name.toLowerCase() === name.toLowerCase() ||
            q.id === norm ||
            q.name.toLowerCase().includes(name.toLowerCase())));
}
/** All registered quest IDs */
export function listRegisteredQuests() {

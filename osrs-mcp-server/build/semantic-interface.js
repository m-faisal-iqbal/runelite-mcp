...
function widgetActions(widget) {
    return Array.isArray(widget?.actions)
        ? widget.actions.filter((action) => typeof action === "string" && action.trim().length > 0)
        : [];
}
function widgetScreen(widget) {
...
function widgetAction(controlType, widget, option) {
...
function addDialogueControls(snapshot, controls) {
...
function addInventoryControls(snapshot, controls) {
...
function addEquipmentControls(snapshot, controls) {
    for (const item of snapshot.equipment ?? []) {
        ...
        addControl(controls, {
            ...
            confidence: 0.85,
            source: "equipment",
            widget: { itemId: item.id, itemName: item.name, slot: item.slot, actions: item.actions },
            screen: { x: item.screenX, y: item.screenY, coordinateSource: item.coordinateSource ?? "equipmentSlot" },
        });
    }
}
function addWidgetControls(widgets, controls) {
...
function groupCounts(controls) {
    return controls.reduce((groups, control) => {
        groups[control.type] = (groups[control.type] ?? 0) + 1;
        return groups;
    }, {});
}
export function buildSemanticInterface(snapshot, widgets = []) {
    if (!snapshot) {
        return {
            status: "NO_SNAPSHOT",
...
export function findSemanticControls(interfaceState, args) {
...
export function planQuestStep(args) {
    ...
    const inventoryNames = new Set((snapshot?.inventory ?? [])
        ...
        .map((item) => cleanUiText(item.name).toLowerCase()));

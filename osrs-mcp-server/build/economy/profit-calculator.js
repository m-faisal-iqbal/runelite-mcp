...
import { getPriceByName, calcTax } from "./ge-price-cache.js";
// ─── Skilling profit methods (curated) ────────────────────────────────────────
const SKILLING_METHODS = [
    {
...
export function getSkillingMethods(skill, category) {
    return SKILLING_METHODS.filter((m) => {
        if (skill && m.skill?.toLowerCase() !== skill.toLowerCase())
            return false;
...
export function analyseFlip(itemName, overrideBuy, overrideSell) {
...
export function calcSessionProfit(gpStart, gpEnd, itemsCollected) {
    ...
    return {
        ...
        itemValue,
        totalProfit,
        profitPerHour: (mins) => Math.round((totalProfit / mins) * 60),
    };
}

...
export function selectOffensivePrayer(style, prayerLevel) {
    const candidates = Object.keys(PRAYERS).filter((k) => {
        const p = PRAYERS[k];
...
export function selectProtectionPrayer(monsterAttackStyle) {
    const map = {
 ...
 */
export function prayerDrainRate(activePrayers, prayerBonus) {
...
export function ticksUntilPrayerDepleted(currentPoints, activePrayers, prayerBonus) {
...
export function getPrayerMultipliers(activePrayers) {
    let atk = 1, str = 1, def = 1, rAtk = 1, rStr = 1, mag = 1;
    for (const name of activePrayers) {
        switch (name) {
            case "BURST_OF_STRENGTH":
                str = Math.max(str, 1.05);
                break;
            case "SUPERHUMAN_STRENGTH":
                str = Math.max(str, 1.10);
                break;
            case "ULTIMATE_STRENGTH":
                str = Math.max(str, 1.15);
                break;
            case "CLARITY_OF_THOUGHT":
                atk = Math.max(atk, 1.05);
                break;
            case "IMPROVED_REFLEXES":
                atk = Math.max(atk, 1.10);
                break;
            case "INCREDIBLE_REFLEXES":
                atk = Math.max(atk, 1.15);
                break;
            case "THICK_SKIN":
                def = Math.max(def, 1.05);
                break;
            case "ROCK_SKIN":
                def = Math.max(def, 1.10);
                break;
            case "STEEL_SKIN":
                def = Math.max(def, 1.15);
                break;
            case "CHIVALRY":
                atk = Math.max(atk, 1.15);
                str = Math.max(str, 1.18);
                def = Math.max(def, 1.20);
                break;
            case "PIETY":
                atk = Math.max(atk, 1.20);
                str = Math.max(str, 1.23);
                def = Math.max(def, 1.25);
                break;
            case "SHARP_EYE":
                rAtk = Math.max(rAtk, 1.05);
                rStr = Math.max(rStr, 1.05);
                break;
            case "HAWK_EYE":
                rAtk = Math.max(rAtk, 1.10);
                rStr = Math.max(rStr, 1.10);
                break;
            case "EAGLE_EYE":
                rAtk = Math.max(rAtk, 1.15);
                rStr = Math.max(rStr, 1.15);
                break;
            case "RIGOUR":
                rAtk = Math.max(rAtk, 1.20);
                rStr = Math.max(rStr, 1.23);
                def = Math.max(def, 1.25);
                break;
            case "MYSTIC_WILL":
                mag = Math.max(mag, 1.05);
                break;
            case "MYSTIC_LORE":
                mag = Math.max(mag, 1.10);
                break;
            case "MYSTIC_MIGHT":
                mag = Math.max(mag, 1.15);
                break;
            case "AUGURY":
                mag = Math.max(mag, 1.25);
                def = Math.max(def, 1.25);
                break;
        }
    }

...
export function defaultLootConfig() {
    return {
        rules: [
...
export function shouldLoot(itemName, itemId, quantity, config, getPrice) {
    ...
    // Coins always
    if (config.alwaysLootCoins && (name === "coins" || name === "coin"))
        return true;
    // Bones
    if (config.alwaysLootBones && BONE_NAMES.has(name))
        return true;
    // Runes / ammo
    if (config.alwaysLootRunes && RUNE_PATTERN.test(name))
        return true;
    // Seeds
    if (config.alwaysLootSeeds && name.includes("seed"))
        return true;
    // Junk exclusion
    if (config.neverLootJunk && JUNK_ITEMS.has(name))
        return false;
    // Default: value threshold
    const price = getPrice(itemId);
    if (!price?.high && !price?.low)
        return false; // untradeable — skip unless ruled

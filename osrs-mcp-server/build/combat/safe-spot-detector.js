 ...
 */
export function isSafeSpot(playerTile, npcTile, // top-left corner of NPC's tile block
npcSize, npcAttackRange = 1, // 1 = melee, >1 = ranged
collisionData) {
...
function isTileBlocked(collisionData, tile) {
 ...
 */
export function findSafeSpot(playerTile, npcTile, npcSize, searchRadius = 8, aggroRange = 10, collisionData) {
    const meleeReach = npcSize;
    ...
    for (let r = 1; r <= searchRadius; r++) {
        for (let dx = -r; dx <= r; dx++) {
            for (let dy = -r; dy <= r; dy++) {
                ...
                if (distToNpc <= meleeReach)
                    continue;
                if (distToNpc > aggroRange)
                    continue;
                if (collisionData && isTileBlocked(collisionData, candidate))
                    continue;
                return candidate;
            }
        }
...
export function rankSafeSpots(candidates, npcTile, npcSize, collisionData) {
    return candidates
        .filter(t => {

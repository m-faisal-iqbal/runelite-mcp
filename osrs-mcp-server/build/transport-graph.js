...
function parseLevelRequirement(requirements, skill) {
    if (!requirements)
        return undefined;
...
function findItemRequirement(requirements) {
...
// ---------------------------------------------------------------------------
export function filterEdgeForPlayer(edge, state) {
    ...
    switch (edge.mode) {
        ...
        case "teleport_item": {
            ...
            if (requiredItem !== undefined) {
                ...
                const hasItem = (state.availableItems ?? []).some((item) => item.toLowerCase().trim() === normalizedRequired);
                if (!hasItem) {
                    return false;
                }
            }
        ...
        case "fairy_ring": {
            if (state.completedQuests !== undefined) {
                const hasQuest = state.completedQuests.some((q) => q.toLowerCase().includes("fairytale ii") || q.toLowerCase().includes("cure a queen"));
                if (!hasQuest) {
                    return false;
                }
            }
        ...
        case "spirit_tree": {
            if (state.completedQuests !== undefined) {
                const hasQuest = state.completedQuests.some((q) => q.toLowerCase().includes("tree gnome village"));
                if (!hasQuest) {
                    return false;
                }
            }
        ...
        case "gnome_glider": {
            if (state.completedQuests !== undefined) {
                const hasQuest = state.completedQuests.some((q) => q.toLowerCase().includes("the grand tree"));
                if (!hasQuest) {
                    return false;
                }
            }
...
export function findTransportNode(query) {
    ...
    return (data.nodes.find((node) => normalize(node.id) === normalized ||
        ...
        data.nodes.find((node) => normalize(node.id).includes(normalized) ||
...
export function nearestTransportNode(location) {
    ...
    return [...data.nodes].sort((a, b) => tileDistance(location, a.worldX, a.worldY, a.plane) -
        tileDistance(location, b.worldX, b.worldY, b.plane))[0];
}
/** Return all outbound edges from a given node. */
export function getEdgesFromNode(nodeId) {
...
// ---------------------------------------------------------------------------
function buildGraph(playerState) {
...
function dijkstra(graph, source, target) {
    ...
    while (unsettled.size > 0) {
        ...
        graph.forEachOutboundEdge(current, (edge) => {

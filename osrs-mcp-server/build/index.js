...
async function getPathfindingStatusForBase(baseURL) {
    ...
    try {
        ...
        const oldPluginPathStatusFallback = error?.response?.status === 404 ||
...
async function assertClientReady(baseURL, options = {}) {
...
async function assertClientLoggedIn(baseURL, identity) {
...
function canvasCaptureRect(debug, args) {
    ...
    if (Number.isFinite(args.canvasX) && Number.isFinite(args.canvasY)) {
        ...
        left = clampInt(args.canvasX - radius, 0, Math.max(0, canvasWidth - 1));
        top = clampInt(args.canvasY - radius, 0, Math.max(0, canvasHeight - 1));
...
async function captureCanvasImage(baseURL, args) {
    ...
    const filePath = await screen.captureRegion(fileBase, new Region(capture.screen.x, capture.screen.y, capture.screen.width, capture.screen.height), FileType.PNG, outputDir);
    const response = {
        filePath,
        mimeType: "image/png",
        capturedAt: Date.now(),
...
function hasDirectMenuParams(target) {
...
function scenePoiDefaultOption(poi, requestedOption) {
    if (requestedOption) {
...
async function scenePoiObjectTargets(baseURL, name, id, option) {
    ...
    return icons
        .filter((poi) => poi?.type === "scenePoi")
        .filter((poi) => targetMatches(poi, name, id))
        .map((poi) => {
...
async function selectActionTarget(baseURL, snapshot, entityType, name, id, nearestToPlayer, coordinateSource, option) {
    ...
    const playerLocation = nearestToPlayer ? snapshot.state?.location : null;
    const fallbackTargets = sortNearestToPlayer(sortByDistance(await scenePoiObjectTargets(baseURL, name, id, option)), playerLocation);
    return fallbackTargets.find((candidate) => hasDirectMenuParams(candidate));
}
...
server.registerResource("latest-snapshot", "osrs://snapshot/latest", {
...
server.registerResource("client-identity", "osrs://client/identity", {
...
server.registerResource("transport-graph", "osrs://transport/graph", {
...
server.registerPrompt("experienced-player-loop", {
...
server.registerPrompt("woodcut-and-bank", {
...
server.registerPrompt("complete-dialogue", {
...
server.registerPrompt("withdraw-and-equip", {
...
async function openContextMenuForTarget(args) {
    ...
    const target = await selectActionTarget(baseURL, snapshot, args.entityType, args.name, args.id, args.nearestToPlayer ?? true, args.coordinateSource);
...
async function interactWithTarget(args) {
    const targetClient = { instanceId: args.instanceId, playerName: args.playerName, port: args.port };
    const { baseURL, snapshot } = await getSnapshotForTarget(targetClient, true);
    await assertClientLoggedIn(baseURL);
    const expectedSource = args.coordinateSource ?? defaultCoordinateSourceForType(args.entityType);
    const target = await selectActionTarget(baseURL, snapshot, args.entityType, args.name, args.id, args.nearestToPlayer ?? true, args.coordinateSource, args.option);
    if (!target) {
        return {
...
function visibleEntityCount(snapshot, entityType, name, id) {
    if (!entityType) {
        return 0;
    }
    return targetsForType(snapshot, entityType).filter((target) => targetMatches(target, name, id)).length;
}
function buildActionVerification(snapshot, args) {
    const checks = [];
    ...
    if (Number.isFinite(args.expectedWorldX) && Number.isFinite(args.expectedWorldY)) {
        const distance = tileDistance(location, args.expectedWorldX, args.expectedWorldY, args.expectedPlane);
        checks.push({
            name: "location",
    ...
    if (args.inventoryItemName || Number.isFinite(args.inventoryItemId) || Number.isFinite(args.inventoryQuantityAtLeast) || Number.isFinite(args.inventoryQuantityChangedFrom)) {
        ...
        const okAtLeast = !Number.isFinite(args.inventoryQuantityAtLeast) || quantity >= args.inventoryQuantityAtLeast;
    ...
    const summary = {
        state,
        inventorySlotsUsed: (snapshot.inventory ?? []).filter((item) => item && item.id && item.id !== -1).length,
...
function snapshotDiffSummary(snapshot, itemName, itemId) {
...
function buildSnapshotDiffVerification(before, after, baselineCapturedAt, args) {
    ...
    const inventoryItemDelta = Number.isFinite(beforeItemQuantity) && Number.isFinite(afterItemQuantity)
        ? afterItemQuantity - beforeItemQuantity
        : undefined;
    ...
    const freshMessages = recentMessages(after).filter((message) => Number(message.capturedAt ?? 0) >= baselineCapturedAt);
    ...
    if (Number.isFinite(args.expectedWorldX) && Number.isFinite(args.expectedWorldY)) {
        const distance = tileDistance(afterLocation, args.expectedWorldX, args.expectedWorldY, args.expectedPlane);
    ...
    if (args.expectNewChat || args.chatContains) {
        const match = args.chatContains
            ? freshMessages.find((message) => matchesChatMessage(message, args.chatContains, args.caseSensitive, args.chatType))
            : freshMessages[0];
...
function matchesOption(actual, requested) {
...
function directMenuActionForTarget(target, option) {
    const actions = Array.isArray(target.menuActions) ? target.menuActions : [];
    const matchedAction = actions.find((action) => matchesOption(action?.option, option));
...
async function validatePreparedStep(baseURL, snapshot, step, args) {
    ...
    if (tool === "deposit_inventory_item" || tool === "withdraw_bank_item") {
        ...
        const match = collection.find((item) => (stepArgs.itemId === undefined || item.id === stepArgs.itemId) &&
    ...
    if (tool === "drop_inventory_item") {
        const item = findInventoryItem(snapshot, stepArgs.name, stepArgs.id, stepArgs.slot);
...
function stepTargetArgs(stepArgs, target) {
    return {
        ...stepArgs,
...
function clickToolEntityType(tool) {
    switch (tool) {
        case "click_object":
            return "object";
        case "click_npc":
...
async function dropInventoryItemAction(args) {
    ...
    const inventoryActions = Array.isArray(item.menuActions) ? item.menuActions : [];
    const dropActionMetadata = inventoryActions.find((action) => matchesOption(action?.option, "Drop"));
    ...
    if (!directAction && Number.isFinite(item.slot) && Number.isFinite(item.inventoryWidgetId) && Number.isFinite(item.id)) {
        directAction = {
            param0: Number(item.slot),
            param1: Number(item.inventoryWidgetId),
            menuAction: "CC_OP_LOW_PRIORITY",
            identifier: 7,
...
async function dryRunAgentStep(baseURL, step, target) {
    const tool = step?.tool;
...
async function executeAgentStepAction(baseURL, step, target, args) {
    const tool = step?.tool;
    const stepArgs = stepTargetArgs(step?.arguments ?? {}, target);
...
async function runEatFoodStep(baseURL, step, target, mode) {
    const stepArgs = stepTargetArgs(step.arguments ?? {}, target);
    const snapshot = await getSnapshotForBase(baseURL, true);
...
async function navigateToDestinationAction(args) {
    ...
    try {
        ...
        const wait = await waitUntilBaseLocation(baseURL, localStep.worldX, localStep.worldY, localStep.plane, localStep.final ? Math.max(1, args.waypointRadius ?? 3) : 2, 8000, 500);
        action = {
...
async function runReflexPolicyStep(step, policy, mode) {
...
function requestedObjectiveCount(objective) {
...
function objectiveInventoryItem(objective) {
...
function initializeSessionGoalState(session, snapshot) {
...
function evaluateSessionGoal(session, snapshot) {
...
async function prepareAutonomyStep(args) {
    ...
    return {
        ...
        baseURL,
        targetClient: resolved.targetClient,
        runtime,
        pathfindingStatus,
        snapshot,
        context,
...
function normalizedMiningItemName(itemName) {
...
function resolveAcquisitionPlan(itemName, method) {
...
function loadReflexPolicyResponse(args) {
...
// --- State Reading Tools ---
server.tool("load_policy", "Load a high-level System 1 Reflex Engine policy. This is the policy entrypoint for System 2; it starts a local 600ms tick loop and never calls the LLM.", {
...
server.tool("reflex_status", "Inspect the active System 1 Reflex Engine policy and recent local tick history.", {}, async () => ({ content: [{ type: "text", text: JSON.stringify(reflexEngine.status(), null, 2) }] }));
server.tool("reflex_pause", "Pause the active System 1 Reflex Engine policy without unloading it.", {
    reason: z.string().optional(),
}, async ({ reason }) => ({ content: [{ type: "text", text: JSON.stringify(reflexEngine.pause(reason), null, 2) }] }));
server.tool("reflex_resume", "Resume a loaded or paused System 1 Reflex Engine policy.", {}, async () => {
    try {
...
server.tool("reflex_stop", "Stop the active System 1 Reflex Engine policy.", {
    reason: z.string().optional(),
}, async ({ reason }) => ({ content: [{ type: "text", text: JSON.stringify(reflexEngine.stop(reason), null, 2) }] }));
server.tool("reflex_tick_once", "Run one immediate System 1 Reflex Engine tick for the active running policy. Intended for tests/debugging; normal policies tick locally on their own timer.", {}, async () => {
    try {
...
server.tool("reflex_history", "Read recent System 1 Reflex Engine events.", {
...
server.tool("observe_game", "Gather a read-only agent observation bundle: compact state, runtime health, optional plan/package, and optional screenshot metadata/image. Never executes gameplay actions.", {
...
server.tool("prepare_agent_step", "Prepare the next observe-plan-act-verify step package for an objective without executing anything. Returns context, plan preview, first action, safety flags, and verification guidance.", {
...
server.tool("validate_prepared_step", "Validate a prepared next step against the current RuneLite state without executing it. Raw invoke_* actions are validated with dryRun by default.", {
...
server.tool("run_agent_cycle", "Run one safe observe-plan-validate cycle for an objective without executing the chosen action. This is the control-loop primitive to call before any real play step.", {
    ...
    radius: z.number().optional().describe("Optional screenshot crop radius around canvasX/canvasY, default 140 canvas pixels"),
    ...clientTargetSchema(),
}, async ({ objective, stepChoice, includeDiagnostics, includeNearbyLimit, includeInventoryLimit, maxPreviewSteps, dryRunRawActions, maxAgeMs, includeScreenshot, includeImage, canvasX, canvasY, width, height, radius, instanceId, playerName, port, }) => {
    try {
        const clients = await discoverClients();
        if (clients.length === 0) {
            const plan = {
...
server.tool("execute_agent_step", "Validate and optionally execute exactly one prepared gameplay step. Defaults to dry-run; real execution requires confirmExecution='EXECUTE_ONE_STEP'.", {
    ...
    ...clientTargetSchema(),
}, async ({ objective, step, stepChoice, executionMode, confirmExecution, baselineBeforeAction, includePostActionSnapshot, includeDiagnostics, includeNearbyLimit, includeInventoryLimit, maxPreviewSteps, maxAgeMs, tickAligned, tickTimeoutMs, instanceId, playerName, port, }) => {
    try {
        const wantsExecution = executionMode === "execute";
        const clients = await discoverClients();
        if (clients.length === 0) {
        ...
        const postActionSummary = postActionSnapshot
            ? {
                capturedAt: postActionSnapshot.capturedAt,
                ageMs: postActionSnapshot.ageMs,
                tick: postActionSnapshot.tick,
...
server.registerResource("strategy-cache", "osrs://memory/strategy-cache", {
...
server.tool("strategy_cache_get", "Lookup a cached strategist policy by key or normalized goal/method before calling Qwen.", {
    ...
    const cacheKey = key ?? strategyCacheKey({ goal: goal, method });
...
server.tool("strategy_cache_put", "Store or replace a cached strategist policy and concrete System 1 policy payload.", {
...
server.tool("strategy_cache_list", "List cached strategy policies for System 2 reuse and inspection.", {
...
server.tool("strategy_cache_record_outcome", "Record whether a cached strategy succeeded or failed after System 1 execution.", {
...
server.tool("memory_search_lessons", "Search persisted memory journal for similar lessons, failures, blockers, and strategy notes before planning a risky goal.", {
...
server.tool("agent_run_goal", `Run a bounded Phase 2 observe-plan-execute-verify loop for an agent goal. Defaults to dry-run; real execution requires confirmExecution='${RUN_AUTONOMY_CONFIRMATION}'.`, {
    ...
    try {
        ...
        for (let index = 0; index < stepLimit; index += 1) {
            ...
            const afterSnapshot = await getSnapshotForBase(prepared.baseURL, true);
...
server.tool("skill_acquire", "Acquire an item through a bounded universal activity. Phase 1 supports woodcutting logs and mining ores.", {
...
server.tool("get_pathfinding_status", "Report current pathfinding capability: loaded-scene collision-map A*, global routing availability, and Shortest Path bridge readiness.", {
    ...clientTargetSchema(),
}, async ({ instanceId, playerName, port }) => {
    try {
...
server.tool("capture_canvas_screenshot", "Capture the current RuneLite canvas, or a crop around canvasX/canvasY, as a PNG for visual verification before/after actions.", {
    ...clientTargetSchema(),
...
server.tool("get_screenshot", "Capture the current RuneLite canvas as a PNG image or metadata-only screenshot result. Alias-friendly perception tool for agents.", {
    ...clientTargetSchema(),
    canvasX: z.number().optional().describe("Optional RuneLite canvas-relative X coordinate. With canvasY, captures a crop around this point."),
    canvasY: z.number().optional().describe("Optional RuneLite canvas-relative Y coordinate. With canvasX, captures a crop around this point."),
    width: z.number().optional().describe("Optional canvas-relative crop width. Used with canvasX/canvasY as the top-left if radius is omitted."),
...
server.tool("get_current_location", "Alias for get_player_location. Get the current player name, world location, health, run energy, tick, capturedAt, and ageMs.", {
    ...clientTargetSchema(),
}, async ({ instanceId, playerName, port }) => {
    try {
        const { snapshot } = await getSnapshotForTarget({ instanceId, playerName, port });
...
server.tool("transport_graph_status", "Inspect the curated System 1 global transport graph used for named-location navigation.", {}, async () => ({
    content: [{
            type: "text",
            text: JSON.stringify({
                status: "TRANSPORT_GRAPH_READY",
...
server.tool("plan_route", "Plan a global named-location route over the System 1 transport graph without clicking or walking.", {
...
server.tool("navigate_to", `Plan a System 1 route to a named destination and optionally execute one bounded movement step. Execution requires confirmExecution='${NAVIGATE_EXECUTE_CONFIRMATION}'.`, {
...
server.tool("perceive_minimap", "Read compact minimap/camera context for System 1 active perception. Optionally projects one destination tile.", {
    worldX: z.number().optional(),
    worldY: z.number().optional(),
    plane: z.number().optional(),
...
server.tool("perceive_chat", "Read compact recent chat/game messages for System 1 active perception.", {
    ...
    try {
        ...
        const filtered = messages.filter((message) => (!type || String(message.type ?? "") === type) &&
...
server.tool("perceive_ui_region", "Read compact UI/interface perception with semantic controls and optional bounded widget filtering.", {
...
server.tool("invoke_walk_action", "Invoke a RuneLite WALK menu action inside the client for a loaded-scene world tile. This is not long-distance pathfinding.", {
...
server.tool("invoke_widget_action", "Invoke a RuneLite widget menu action inside the client. Prefer raw params from a widget/menu entry; otherwise use packedId or groupId+childId with actionIndex 1-5.", {
...
server.tool("interact_with", "Interact with a fresh NPC, object, player, or ground item by opening its context menu and invoking the selected RuneLite menu action in-client.", {
...
server.tool("walk_to", "Walk to a nearby world tile. In auto mode, try an in-client WALK action for loaded-scene tiles before falling back to minimap click.", {
...
server.tool("walk_path_to", "Walk one bounded step toward a target world tile using local collision-aware in-client walking only. Does not use hardware mouse or minimap fallback.", {
    worldX: z.number().describe("Target world X tile"),
    worldY: z.number().describe("Target world Y tile"),
    plane: z.number().optional().describe("Target plane, defaults to the player's current plane"),
...
server.tool("verify_after_action", "Poll fresh snapshots until expected post-action evidence is observed, then return a compact verification report.", {
    ...
    ...clientTargetSchema(),
}, async ({ timeoutMs, pollMs, expectIdle, expectedWorldX, expectedWorldY, expectedPlane, locationRadius, chatContains, chatType, caseSensitive, inventoryItemName, inventoryItemId, inventoryQuantityAtLeast, inventoryQuantityChangedFrom, entityType, entityName, entityId, requireEntityVisible, dialogueType, instanceId, playerName, port, }) => {
    const startedAt = Date.now();
...
server.tool("mark_action_baseline", "Capture the current snapshot as the before-state for verify_last_action. Use immediately before a risky click, in-client action, skilling loop step, or UI interaction.", {
...
server.tool("verify_last_action", "Compare the latest fresh snapshot against the most recent mark_action_baseline snapshot, optionally polling until requested deltas appear.", {
    timeoutMs: z.number().optional().describe("Maximum wait time in milliseconds, default 8000"),
    pollMs: z.number().optional().describe("Polling interval in milliseconds, default 500"),
    ...
    ...clientTargetSchema(),
}, async ({ timeoutMs, pollMs, inventoryItemName, inventoryItemId, expectInventoryQuantityChanged, expectInventoryIncreased, expectInventoryDecreased, expectInventorySlotsChanged, expectLocationChanged, expectedWorldX, expectedWorldY, expectedPlane, locationRadius, expectDialogueChanged, dialogueType, expectNewChat, chatContains, chatType, caseSensitive, entityType, entityName, entityId, expectEntityCountChanged, requireAnyChange, clearBaseline, instanceId, playerName, port, }) => {
    const startedAt = Date.now();
    const timeout = Math.max(1, timeoutMs ?? 8000);
    const interval = Math.max(100, pollMs ?? 500);
    let lastReport = null;
...
server.tool("diagnose_runtime", "Check active RuneLite MCP plugin runtimes for stale jars, missing endpoints, and feature flags before attempting gameplay actions.", {
    ...clientTargetSchema(),
}, async ({ instanceId, playerName, port }) => {
    try {
        const clients = await discoverClients();
        const selectedClients = clients.filter((client) => (port !== undefined && client.port === port) ||
            (instanceId && client.instanceId === instanceId) ||
            (playerName && String(client.playerName ?? "").toLowerCase() === playerName.toLowerCase()) ||

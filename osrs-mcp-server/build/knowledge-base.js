...
export function getKnowledgeRecord(kind, idOrName) {
    const needle = normalize(idOrName).replace(/\s+/g, "_");
    return allKnowledgeRecords().find((record) => record.kind === kind &&
        (record.id.toLowerCase() === needle || normalize(record.name) === normalize(idOrName)));
}
function parseTargetLevel(text, explicit) {
...
export function getMethodKnowledge(args) {
    if (args.methodId) {
        ...
        return record ? methodWithPlan(record, args) : undefined;
    }
...
function methodWithPlan(method, args) {
